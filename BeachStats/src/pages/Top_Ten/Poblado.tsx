...
import InfoPoblado from "../../components/top_ten/InfoPoblado";
import useTopTypes from "../../customHooks/useTopTypes";


export default function Poblado() {
  const { beaches, loading, error } = useTopTypes("DE_POBLADO");
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-100 w-full">
      <div className="w-full md:w-4/5 flex flex-col px-2 sm:px-4 md:px-6">
        <div className="flex flex-col w-full gap-4 lg:gap-8 py-4">
          ...
          <InfoPoblado></InfoPoblado>
          <Best10Beaches beach={beaches} loading={loading} error={error}></Best10Beaches>
        </div>
